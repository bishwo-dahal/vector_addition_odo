...
int main(int argc, char *argv[]){
    ...
    #pragma omp parallel default(shared) firstprivate(rank)
    {
    int hwthread           = sched_getcpu();
    int omp_thread_id      = omp_get_thread_num();
    int omp_num_threads    = omp_get_num_threads();
    int omp_chunk_size     = chunk_size / omp_num_threads;   
    int lower_bound        = omp_thread_id * omp_chunk_size;
    size_t omp_buffer_size = omp_chunk_size * sizeof(double);
 
    vec_add_on_gpu(rank, omp_thread_id, hwthread, node_name, sub_A+lower_bound, sub_B+lower_bound, sub_C+lower_bound, omp_buffer_size, omp_chunk_size);
    }
